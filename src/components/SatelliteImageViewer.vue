...
function applySmartCropping() {
  ...
  // 判断太阳是否在北半球（3月21日至9月23日之间）
  const isSunInNorthernHemisphere = () => {
  ...
  const isNorthern = isSunInNorthernHemisphere()
  console.log(`[applySmartCropping] 太阳是否在北半球: ${isNorthern}`)
  console.log(
  ...
  if (imgScaledWidth > containerWidth) {
    ...
    if (isEarlyMorning) {
      ...
      if (isNorthern) {
        offsetY = blackBorderSize + blackBorderSize // 北半球显示顶部
      }
    } else {
      offsetX = 0 + blackBorderSize + blackBorderSize // 其他时间
      if (!isNorthern) {
        offsetY = -blackBorderSize - blackBorderSize // 南半球显示底部
      } else {
        offsetY = blackBorderSize // 北半球显示顶部
      }
    }
    console.log(
  ...
  if (imgScaledHeight > containerHeight) {
    ...
    if (isNorthern) {
      ...
      offsetY = -overflowHeight - blackBorderSize
    }
    // 始终显示顶部
