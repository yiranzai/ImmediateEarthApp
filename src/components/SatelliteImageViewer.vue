<template>
  <div class="relative w-screen h-screen overflow-hidden bg-black">
    ...
    <div class="absolute inset-0 flex flex-col justify-center items-center pointer-events-none">
      <div class="text-white text-center text-shadow-md px-6 py-4">
        <div class="text-[clamp(3rem,10vw,6rem)] font-light mb-2">{{ formatDate(currentTime, 'HH:mm') }}</div>
        <div class="text-[clamp(1rem,3vw,1.5rem)] opacity-90">{{ formatDate(currentTime, 'yyyy-MM-dd EEEE') }}</div>
      </div>
    </div>
  </div>
</template>
...
const getImageTime = () => {
  const now = new Date();
  now.setUTCMinutes(now.getUTCMinutes() - 30); // 减去30分钟以确保数据可用
...
const applySmartCropping = () => {
  ...
  console.log(`[applySmartCropping] 日本时间: ${japanTime.toLocaleString()}, 小时: ${hours}, 下午: ${isAfternoon}, 凌晨: ${isEarlyMorning}`);

  // 获取容器和canvas元素
  const container = document.getElementById('satellite-container');
  const canvas = canvasRef.value;
  console.log(`[applySmartCropping] 容器元素: ${container ? '找到' : '未找到'}, Canvas元素: ${canvas ? '找到' : '未找到'}`);
  ...
  console.log(`[applySmartCropping] 容器尺寸: ${containerWidth}x${containerHeight}, 图像尺寸: ${imgNaturalWidth}x${imgNaturalHeight}`);
  ...
  console.log(`[applySmartCropping] 缩放比例: ${scale.toFixed(4)}, 缩放后图像尺寸: ${Math.round(imgScaledWidth)}x${Math.round(imgScaledHeight)}`);
 ...
 if (isEarlyMorning) {
    ...
    console.log(`[applySmartCropping] 水平裁剪 - 溢出宽度: ${Math.round(overflowWidth)}px, 偏移X: ${Math.round(offsetX)}px`);
  }

  // 垂直方向偏移（上下裁剪）- 仅当图片高度大于容器高度时
  if (imgScaledHeight > containerHeight) {
    const overflowHeight = imgScaledHeight - containerHeight;
    const overflowWidth = imgScaledWidth - containerWidth;
    if (isAfternoon) {
      ...
      offsetX = -overflowWidth / 2; // 默认显示中间
      offsetY = blackBorderSize
    } // 始终显示顶部
  ...
  console.log(`[applySmartCropping] 应用样式 - width: ${imgScaledWidth}px, height: ${imgScaledHeight}px, transform: translate(${offsetX}px, ${offsetY}px)`);
};
